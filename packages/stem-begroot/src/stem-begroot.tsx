...
function StemBegroot({
	...
	const notifyVoteMessage = (message: string, isError = false) => {
		if (isError) {
			...
			<StemBegrootResourceDetailDialog
				...
				onPrimaryButtonClick={(resource) => {
					if (
						...
						if (activeTagTab) {
							const activeTag = tagCounter.find(
								(tagObj) => tagObj[activeTagTab],
							);
							if (activeTag) {
								const selectedResources =
									activeTag[activeTagTab].selectedResources;
								const resourceInTagList = selectedResources.find(
									(r) => r.id === resource.id,
								);
								...
								if (resourceInTagList) {
									...
									activeTag[activeTagTab].current -=
										props.votes.voteType === "budgetingPerTag"
											? resource.budget
											: 1;
								} else {
										...
										setTagCounter((prevTagCounter) => {
											return prevTagCounter.map((tagObj) => {
												if (tagObj[activeTagTab]) {
													const updatedSelectedResources = tagObj[
														activeTagTab
