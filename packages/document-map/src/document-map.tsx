...
function DocumentMap({
	...
	const characterHelpText = (count: number) => {
		...
		const minWarning = `Nog minimaal ${min - count} karakters`;

		const max =
			props.comments?.descriptionMaxLength || Number.POSITIVE_INFINITY;
															...
															{group?.multiple ? (
																...
																<SelectField
																	...
																	onChange={(e: {
																		...
																		const selectedTag = e.value as string;

																		updateTagListMultiple(
																			Number.parseInt(selectedTag, 10),
																		);
																	}}
																/>
															)}
