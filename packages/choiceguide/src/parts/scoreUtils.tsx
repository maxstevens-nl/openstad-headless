...
export const calculateColor = (
	...
	if (maxColorMatch && minColorMatch) {
		const r =
			Number.parseInt(minColorMatch[1], 16) +
			(Number.parseInt(maxColorMatch[1], 16) -
				Number.parseInt(minColorMatch[1], 16)) *
				(score / 100);
		const g =
			Number.parseInt(minColorMatch[2], 16) +
			(Number.parseInt(maxColorMatch[2], 16) -
				Number.parseInt(minColorMatch[2], 16)) *
				(score / 100);
		const b =
			Number.parseInt(minColorMatch[3], 16) +
			(Number.parseInt(maxColorMatch[3], 16) -
				Number.parseInt(minColorMatch[3], 16)) *
				(score / 100);
		return `rgb(${r},${g},${b})`;
	}
...
export const calculateScoreForItem = (
	...
	const choiceOptionsArray = Array.isArray(choiceOption)
		? choiceOption
		: [choiceOption];
		...
		Object.keys(weights[option.id] || {}).forEach((answerKey) => {
			...
			Object.keys(optionWeights).forEach((optionId) => {
				...
				["x", "y", "z"].forEach((secondDimension) => {
					if (
						typeof secondOptionWeights[secondDimension] === "number" &&
						secondOptionWeights[secondDimension] !== 0
					) {
						countScores[secondDimension] += Number(
							secondOptionWeights[secondDimension],
						);
					}
				});
			});
