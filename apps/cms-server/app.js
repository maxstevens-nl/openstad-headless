...
async function setupProject(project) {
	...
	const url = Url.parse(project.url);
	...
	if (!subscriptions[project.id]) {
		const subscriber = await messageStreaming.getSubscriber();
		if (subscriber) {
...
async function serveSite(req, res, siteConfig, forceRestart) {
	const dbName = siteConfig.config?.cms?.dbName
	...
	try {
		...
		const redirectURI = siteConfig.config?.cms?.redirectURI;
		if (redirectURI) {
		...
		while (startUpQueue.length) {
			const nextInQueue = startUpQueue.shift();
			const [nextDomain, nextReq, nextRes] = nextInQueue;
 ...
 */
app.use("/:sitePrefix", (req, res, next) => {
...
app.get("/auth/logout", (req, res, next) => {
	const returnUrl = createReturnUrl(req, res);
...
app.use(async (req, res, next) => {
	...
	res
		.status(404)
		.send(
			`Error: No project found for given URL ${req.openstadDomain}${req.url}`,
		);
});
