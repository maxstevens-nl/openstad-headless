...
async function authMiddleware(req: NextRequest, res: NextResponse) {
	...
	if (match) targetProjectId = Number.parseInt(match[1]);
	...
	const openstadlogintoken = searchParams.get("openstadlogintoken");
	...
	if (!req.nextUrl.pathname.startsWith("/_")) {
		...
		if (jwt) {
			try {
				const url = `${process.env.API_URL_INTERNAL || process.env.API_URL}/auth/project/${targetProjectId}/me`;
...
const defaultSession: SessionUserType = {};
