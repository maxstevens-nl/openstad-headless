...
module.exports = function (db, sequelize, DataTypes) {
		...
		"resource",
		{
			...
			budget: {
				...
				set: function (budget) {
					...
					this.setDataValue("budget", Number.parseInt(budget, 10) || null);
				},
			},
							...
							Object.keys(config).forEach((key) => {
								...
								if (value[key]) {
									switch (config[key].type) {
										...
										case "int":
											if (Number.parseInt(value[key]) !== value[key]) {
	...
	Resource.auth = Resource.prototype.auth = {
		...
		toAuthorizedJSON: (user, data, self) => {
	...
	async function beforeValidateHook(instance, options) {
		...
		if (instance.projectId) {
			const project = await db.Project.findByPk(instance.projectId);
		...
		if (
			...
			const firstCommentSubmitted = await db.Comment.count({
				where: { resourceId: instance.id },
			});
