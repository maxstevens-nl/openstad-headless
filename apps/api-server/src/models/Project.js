...
module.exports = (db, sequelize, DataTypes) => {
	var Project = sequelize.define(
		"project",
		{
			...
			emailConfig: {
				...
				defaultValue: {},
				get: function () {
					const value = this.getDataValue("emailConfig");
    ...
    */
			...
			hooks: {
				beforeValidate: async (instance, options) => {
					try {
						...
						const current = await db.Project.findOne({
						...
						if (
							...
							const config = merge.recursive(true, instance.config);
				...
				beforeCreate: (instance, options) =>
					beforeUpdateOrCreate(instance, options),
				...
				beforeDestroy: async (instance, options) => {
					...
					const found = await db.User.findAll({
						where: {
				...
				afterCreate: async (instance, options) => {
					...
					const defaultStatus = await db.Status.create({
						projectId: instance.id,
						name: "open",
						seqnr: 10,
	...
	Project.prototype.willAnonymizeAllUsers = async function () {
		...
		try {
			if (!this.id) throw Error("Project not found");
			if (!this.config.project.projectHasEnded)
				throw Error(
			...
			for (const user of usersToAnonymize) {
				await new Promise((resolve, reject) => {
					setTimeout(async () => {
						...
						user.project = this;
						const res = await user.doAnonymize();
						user.project = null;
	...
	Project.prototype.isVoteActive = function () {
		...
		if (
			...
			voteIsActive =
				moment().isAfter(this.config.votes.isActiveFrom) &&
				moment().isBefore(this.config.votes.isActiveTo);
		}
		return voteIsActive;
	};
