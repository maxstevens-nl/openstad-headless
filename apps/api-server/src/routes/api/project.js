...
async function getProject(req, res, next, include = []) {
	...
	const query = { where: { id: Number.parseInt(projectId) }, include: include };
	try {
		const project = await db.Project.scope(req.scope).findOne(query);
...
router
	...
	.get(async (req, res, next) => {
		try {
			...
			if (!hasRole(req.user, "superuser")) {
				...
				const projectIds = users.map((u) => u.projectId);
				where = { id: projectIds };
			}

			// now find the corresponding projects
			const result = await db.Project.scope(req.scope).findAndCountAll({
			...
			for (const provider of providers) {
				...
				if (!providersDone[authConfig.provider]) {
					...
					if (adapter.service.createClient) {
						const client = await adapter.service.createClient({
							authConfig,
							project,
						});
	...
	.post(async (req, res, next) => {
		const publisher = await messageStreaming.getPublisher();
		if (publisher) {
	...
	.post((req, res, next) => res.json(req.results));
...
router
		...
		try {
			...
			for (const provider of providers) {
				...
				if (adapter.service.updateClient) {
					const merged = merge.recursive({}, authConfig, {
						config: configData,
					});
	...
	.put(async (req, res, next) => {
		...
		if (
			...
			const adminAuthConfig = await authSettings.config({ project: project });
	...
	.put(async (req, res, next) => {
		...
		if (typeof req?.results?.config?.allowedDomains !== "undefined") {
			const proj = req.results.dataValues;
...
router
	...
	.put((req, res, next) => {
		// the project
		const where = { id: Number.parseInt(req.params.projectId) };
...
router.route("/:projectId(\\d+)/css/:componentId?").get((req, res, next) => {
...
router
	...
	.get((req, res, next) => {
		...
		const widgetSettings = widgetDefinitions[req.params.widgetType];
