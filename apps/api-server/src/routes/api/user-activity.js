...
router
	...
	.get((req, res, next) => {
		req.userIds = [Number.parseInt(req.params.userId)];
	...
	.get((req, res, next) => {
		...
		return db.Project.findAll({ where }).then((rows) => {
	...
	.get((req, res, next) => {
		if (!req.activities.includes("resources")) return next();
		const where = { userId: req.userIds };
						...
						? req.results[which].map((instance) => {
								...
								return {
									...
									description: instance[config.descriptionKey]
										? instance[config.descriptionKey].replace(/<[^>]+>/g, "")
										: "",
									type: config.type,
									resource: resource,
