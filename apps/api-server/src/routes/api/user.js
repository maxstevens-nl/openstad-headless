...
router
	...
	.put((req, res, next) => {
		// this user
		req.userId = Number.parseInt(req.params.userId);
...
router
	...
	.put(async (req, res, next) => {
		...
		const userData = merge.recursive(true, req.body);
		...
		try {
			if (user.idpUser?.identifier) {
				...
				const synchronizedUpdatedUserData = merge.recursive(
					{},
					updatedUserData,
				);
				const userProjectSpecificFields = ["nickName", "role"]; // todo: dit moet natuurlijk niet hier, maar dat is nu minder relevant
				for (const userProjectSpecificField of userProjectSpecificFields) {
					delete synchronizedUpdatedUserData[userProjectSpecificField];
				}

				const apiUsers = await db.User.scope(["includeProject"]).findAll({
					where: {
						idpUser: {
				...
				const apiUser = await db.User.scope(["includeProject"]).findOne({
					where: {
						id: user.id,
