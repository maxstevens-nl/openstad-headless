...
router
	...
	.get((req, res, next) => {
		...
		const resourceId = Number.parseInt(req.query.resourceId);
		if (resourceId) {
...
router
	...
	.post((req, res, next) => {
		const ids = req.votes.map((entry) => entry.resourceId);
		const transaction = res.locals.transaction;
		db.Resource.findAll({
	...
	.post((req, res, next) => {
		...
		const err = createError(400, "Aantal resources klopt niet");
		if (transaction) {
			return transaction
				.rollback()
				.then(() => next(err))
	...
	.post((req, res, next) => {
		...
		if (
			(req.project.config.votes.minResources &&
				totalNoOfVotes < req.project.config.votes.minResources) ||
			(req.project.config.votes.maxResources &&
				totalNoOfVotes > req.project.config.votes.maxResources)
		) {
			isOk = false;
		}
