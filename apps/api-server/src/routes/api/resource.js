...
router
	...
	.get((req, res, next) => {
		...
		if (dbQuery.hasOwnProperty("order")) {
			 ...
			 */
			dbQuery.order = dbQuery.order.map((sortingQuery) => {
	...
	.post(async (req, res, next) => {
		...
		const defaultStatusses = await db.Status.findAll({
	...
	.post(async (req, res, next) => {
		...
		const defaultTags = await db.Tag.findAll({
			where: { projectId: req.project.id, addToNewResources: true },
		});
...
router
	...
	.put(async (req, res, next) => {
		...
		resourceInstance.setTags(tagEntities).then((result) => {
			...
			const scope = [...req.scope, "includeTags"];
			if (req.canIncludeVoteCount) scope.push("includeVoteCount");
	...
	.put(async (req, res, next) => {
		...
		resourceInstance.setStatuses(statusEntities).then((result) => {
			...
			const scope = [...req.scope, "includeStatuses"];
			if (req.canIncludeVoteCount) scope.push("includeVoteCount");
			return db.Resource.scope(...scope)
				.findOne({
					where: { id: resourceInstance.id, projectId: req.params.projectId },
