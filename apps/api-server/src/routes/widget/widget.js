...
router
	...
	.get(async (req, res, next) => {
		...
		const widgetDefinitions = getWidgetSettings();
		const widgetSettings = widgetDefinitions[widgetType];
...
function getDefaultConfig(project, widgetType) {
	...
	const url = process.env.IMAGE_APP_URL;
...
function getWidgetJavascriptOutput(
	...
	const extraCssFile = data.project?.cssUrl
		? `<link href="${data.project.cssUrl}" rel="stylesheet">`
		: "";
