								...
								service: {
									...
									port: {
										number: process.env.KUBERNETES_FRONTEND_SERVICE_PORT
											? Number.parseInt(
													process.env.KUBERNETES_FRONTEND_SERVICE_PORT,
												)
											: 4444,
									},
								},
...
const checkHostStatus = async (conditions) => {
	...
	if (isOnK8s) {
		...
		const promises = projects.map(async (project) => {
			...
			hostStatus.dnsRecordIsSetToCorrectIp =
				domainIp !== null && domainIp === serverIp;
			...
			if (hostStatus.dnsRecordIsSetToCorrectIp && !ingress) {
				try {
					...
					console.error(
						`Error creating ingress for ${project.uniqueId} domain: ${project.url} : ${error}`,
					);
				}
