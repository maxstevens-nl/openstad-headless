...
module.exports = {
	...
	paginateResults: function paginateResults(req, res, next) {
		...
		let currentPage = Number.parseInt(req.query.page) || 0;
		let pageSize = req.dbQuery.pageSize; // if defined then paging was done in the query
		const listLength = pageSize ? req.dbQuery.count : list.length;
		let pageCount =
			Number.parseInt(listLength / pageSize) + (listLength % pageSize ? 1 : 0);

		// if (currentPage >= pageCount) currentPage = pageCount - 1;
		if (currentPage < 0) currentPage = 0;

		if (!pageSize) {
			pageSize = Number.parseInt(req.query.pageSize) || 20;
			pageCount =
				Number.parseInt(listLength / pageSize) +
				(listLength % pageSize ? 1 : 0);
