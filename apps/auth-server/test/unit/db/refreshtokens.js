...
describe("refreshTokens", () => {
	...
	describe("#find", () => {
		test("should return empty refresh tokens with invalid token request", () =>
			refreshTokens.find("abc").then((token) => expect(token).toBeUndefined));

		test("should return empty refresh tokens with null", () =>
			refreshTokens.find(null).then((token) => expect(token).toBeUndefined));
	...
	describe("#delete", () => {
		test("should return empty refresh tokens with invalid token request", () =>
			refreshTokens.delete("abc").then((token) => expect(token).toBeUndefined));

		test("should return empty refresh tokens with null", () =>
			refreshTokens.delete(null).then((token) => expect(token).toBeUndefined));
